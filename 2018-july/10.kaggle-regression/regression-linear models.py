...
print(linr_estimator.coef_)
print(linr_estimator.intercept_)
results = model_selection.cross_validate(linr_estimator, X_train1, y_train, cv=10, scoring=metrics.make_scorer(rmse))
results.get('test_score').mean()
results.get('train_score').mean()

ridge_estimator = linear_model.Ridge()
ridge_grid = {'alpha':[0.1,0.5,1.0]}
fit_model(ridge_estimator, ridge_grid, X_train1, y_train)

lasso_estimator = linear_model.Lasso()
lasso_grid = {'alpha':[0.1,0.5,1.0]}
fit_model(lasso_estimator, lasso_grid, X_train1, y_train)
