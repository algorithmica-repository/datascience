...
et_estimator = ensemble.ExtraTreesClassifier(random_state=2017)
et_grid = {'n_estimators':[3], 'max_features':[6,7,8], 'max_depth':[3,4,5]}
grid_et_estimator = model_selection.GridSearchCV(et_estimator, et_grid, cv=10,n_jobs=1)
grid_et_estimator.fit(X_train, y_train)
print(grid_et_estimator.grid_scores_)
print(grid_et_estimator.best_score_)
print(grid_et_estimator.best_params_)
print(grid_et_estimator.score(X_train, y_train))
