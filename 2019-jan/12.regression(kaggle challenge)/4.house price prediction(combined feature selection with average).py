...
svm_grid= {'C':[0.5, 1, 2, 10, 15, 20], 'gamma':[0.001, 0.005, 0.01, 0.05, 0.1, 0.2] }
svm_model = get_best_model1(svm_estimator, svm_grid, X_train1, y_trans, scoring = scoring )

X_test = house3[house_train.shape[0]:]
X_test1 = X_test[list(important_features)]

pred_gb = np.expm1(gb_model.predict(X_test1))
pred_svm = np.expm1(svm_model.predict(X_test1))
house_test['SalePrice'] = (np.array(pred_gb) + np.array(pred_svm)) /2.0
