...
#feature selection via single model
...
plot_feature_importances(feature_selector, X_train1, y_train)

tmp_model1 = feature_selection.SelectFromModel(feature_selector, threshold="median", prefit=True)
print(tmp_model1.get_support())
X_train2 = tmp_model1.transform(X_train)

#feature selection via single model
feature_selector = ensemble.AdaBoostClassifier()
feature_selector.fit(X_train1, y_train)
print(feature_selector.feature_importances_)
plot_feature_importances(feature_selector, X_train1, y_train)

tmp_model2 = feature_selection.SelectFromModel(feature_selector, threshold="median", prefit=True)
print(tmp_model2.get_support())
X_train2 = tmp_model2.transform(X_train)
