#with log transformation of target and kernel svm
import sys
path = 'E://utils'
...
from sklearn import metrics, svm, model_selection, neighbors, linear_model, decomposition, manifold
import math
import pandas as pd
import os
...
kernel_svm_grid  = {'C':[0.2, 0.5, 10, 20, 50], 'gamma':[0.0001, 0.0005, 0.001, 0.005]}
final_kernel_svm_model = utils.grid_search_best_model(kernel_svm, kernel_svm_grid, pca_data, y_trans, scoring = scoring)
...
house_test['SalePrice'] = np.expm1(final_kernel_svm_model.predict(pca_test_data))
