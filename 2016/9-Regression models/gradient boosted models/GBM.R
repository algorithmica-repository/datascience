...
gradient.boosted.tree.learn = function(train.set, iterations, rate=1, max_depth=1) {
  y = train.set[,2]
  x = train.set[,-2]
  avg = mean(y) # start with an initial model
  dy = y - avg  # residuals/error,the parts that existing model cannot do well
  init_model = avg
  ensemble = list() 
  ...
  return(list("ensemble"=ensemble, "init_model"=init_model))
}

gradient.boosted.tree.predict = function(model, test.set, rate=1) {
  y = test.set[,2]
  x = test.set[,-2]
  predict_y = list() 
  predict_y[[1]] = rep(model$init_model,nrow(test.set))
...
model = gradient.boosted.tree.learn(train.set,1000)
model
out = gradient.boosted.tree.predict(model, test.set)
plot(out$mse,type='l')
