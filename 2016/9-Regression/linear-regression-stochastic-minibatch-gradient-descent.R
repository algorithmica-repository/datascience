...
stochastic_gradient = function(points, b_current, m_current) {
  b_gradient = 0
  m_gradient = 0
  N = nrow(points)
  
  #lets take batch size of 10
  random_point_indices = sample(1:N,10)
  for (i in 1:length(random_point_indices)) {
    x = points[random_point_indices[i], 1]
    y = points[random_point_indices[i], 2]
...
stochastic_gradient_descent = function(points, learningRate, iterations) {
  b_current = 0
  m_current = 0
  b_trace = 0
  m_trace = 0
  ...
  for(iter in 1:iterations) {
    grad = stochastic_gradient(points, b_current, m_current)
...
solution = stochastic_gradient_descent(data, 0.0001, 1200)
solution[1:1000,]
plot(1:1201,solution$error,col="blue",xlab="Iterations",ylab="error",main="Convergence")
