...
plot.data = function(X,Y) {
  plot(X[,1],X[,2],col=ifelse(Y == 1,"yellow","blue"),xlab="v1",ylab="v2",main="Logistic Regression")
}
...
plot.error = function(solution,X,Y) {
  ...
  for(i in 1:length(w1)) {
    for(j in 1:length(w2)) {
      error[i,j] = logError(c(w1[i],w2[j]),X,Y)
    }
  }
  persp3D(w1, w2, error, xlab="w1", ylab="w2", zlab="error",col = ramp.col(n = 50, col = c("#FF033E", "#FFBF00", "#FF7E00", "#08E8DE", "#00FFFF", "#03C03C"), alpha = .1), border = "#808080", theta = 60, phi = 20, expand = 0.9, colkey = FALSE, ticktype="detailed")
