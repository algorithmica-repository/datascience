...
y_train_log = house_train['SalePrice'].map(math.log2)
sns.distplot(y_train_log, hist=False)
...
final_estimator = fit_model_neighbors(knn_estimator, knn_grid, X_train1, y_train_log)
...
house_test['SalePrice'] = house_test['SalePrice'].map(lambda x:math.pow(2, x))
