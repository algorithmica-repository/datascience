...
var_obj = remove.nzv.features(restaurant_train)
restaurant_train1 = restaurant_train[, var_obj$zeroVar==FALSE]
...
features_new_df = add.features(restaurant_train1$Open.Date)
restaurant_train2 = cbind(restaurant_train1, features_new_df)
...
model1 = train(x = restaurant_train2[,-features.exclude], y = restaurant_train2[,"revenue"],
...
restaurant_train2 = restaurant_train2[,-features.exclude]
model2 = train(revenue ~ ., data = restaurant_train2, method = "rf", trControl = tr_ctrl, importance=T)
model2$finalModel
getTree(model2$finalModel,1,labelVar = T)
plot(model2)
model2$finalModel
...
features.exclude = c(1,2,3)
restaurant_train2 = restaurant_train2[,-features.exclude]
model3 = train(revenue ~ ., data = restaurant_train2, method = "knn", trControl = tr_ctrl)
model3
plot(model3)
model3$finalModel
...
restaurant_test1 = restaurant_test[, var_obj$zeroVar==FALSE]
dim(restaurant_test1)
str(restaurant_test1)

features_new_df = add.features(restaurant_test1$Open.Date)
restaurant_test2 = cbind(restaurant_test1, features_new_df)
dim(restaurant_test2)
str(restaurant_test2)

restaurant_test2$revenue = predict(model3,restaurant_test2[,-features.exclude])
