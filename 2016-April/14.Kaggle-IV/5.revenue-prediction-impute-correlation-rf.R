...
features.exclude = c(1,2,31,32)
model1 = train(x = restaurant_train4[,-features.exclude], y = restaurant_train4[,"revenue"],
...
restaurant_train4 = restaurant_train4[,-features.exclude]
model2 = train(revenue ~ ., data = restaurant_train4, method = "rf", trControl = tr_ctrl, importance=T)
model2
plot(model2)
model2$finalModel
...
features.exclude = c(1,2,31,32)
restaurant_test4$revenue = predict(model1,restaurant_test4[,-features.exclude])
