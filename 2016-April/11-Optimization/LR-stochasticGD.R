...
stochastic_gradient = function(data, b_current, m_current) {
  N = nrow(data)
...
stochastic_gradient_descent = function(data, learningRate, iterations) {
  b_current = 0
  m_current = 0
  b_trace = b_current
  m_trace = m_current
  ...
  for(iter in 1:iterations) {
    grad = stochastic_gradient(data, b_current, m_current)
...
stepsize = 0.0005
set.seed(100)
solution = stochastic_gradient_descent(data, stepsize, iterations)
